...
class AppPage extends Component {
  ...
  static async getInitialProps({ req }) {
    const response = await fetch(process.env.BASE_URL + "/messages6");
  ...
  componentDidMount() {
    ...
    this.socket.on("messages6", this.handleMessage);
    this.socket.on("clear messages", this.handleClear);
  }

  // close socket connection
  componentWillUnmount() {
    this.socket.off("messages6", this.handleMessage);
    this.socket.close();
  }
  ...
  handleSubmit = (event) => {
    ...
    if (this.state.field.length != 0) {
      ...
      this.socket.emit("messages6", message);
