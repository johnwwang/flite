...
class AppPage extends Component {
  ...
  static async getInitialProps({ req }) {
    const response = await fetch(process.env.BASE_URL + "/messages3");
  ...
  componentDidMount() {
    ...
    this.socket.on("messages3", this.handleMessage);
    this.socket.on("clear messages", this.handleClear);
  }

  // close socket connection
  componentWillUnmount() {
    this.socket.off("messages3", this.handleMessage);
    this.socket.close();
  }
  ...
  handleSubmit = (event) => {
    ...
    if (this.state.field.length != 0) {
      ...
      this.socket.emit("messages3", message);
