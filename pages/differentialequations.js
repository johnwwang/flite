...
class AppPage extends Component {
  ...
  static async getInitialProps({ req }) {
    const response = await fetch(process.env.BASE_URL + "/messages5");
  ...
  componentDidMount() {
    ...
    this.socket.on("messages5", this.handleMessage);
    this.socket.on("clear messages", this.handleClear);
  }

  // close socket connection
  componentWillUnmount() {
    this.socket.off("messages5", this.handleMessage);
    this.socket.close();
  }
  ...
  handleSubmit = (event) => {
    ...
    if (this.state.field.length != 0) {
      ...
      this.socket.emit("messages5", message);
