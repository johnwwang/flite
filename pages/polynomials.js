...
class AppPage extends Component {
  ...
  static async getInitialProps({ req }) {
    const response = await fetch(process.env.BASE_URL + "/messages2");
  ...
  componentDidMount() {
    ...
    this.socket.on("messages2", this.handleMessage);
    this.socket.on("clear messages", this.handleClear);
  }

  // close socket connection
  componentWillUnmount() {
    this.socket.off("messages2", this.handleMessage);
    this.socket.close();
  }
  ...
  handleSubmit = (event) => {
    ...
    if (this.state.field.length != 0) {
      ...
      this.socket.emit("messages2", message);
